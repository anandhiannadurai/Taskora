...
import { Plus, Search, Calendar, Clock, CheckCircle2, Circle, Eye, Sparkles } from 'lucide-react';

const KANBAN_COLUMNS = [
  { id: 'To Do', label: 'To Do', accent: 'border-slate-400', badgeColor: 'bg-slate-500' },
  { id: 'In Progress', label: 'In Progress', accent: 'border-brand-500', badgeColor: 'bg-brand-500' },
  { id: 'Review', label: 'In Review', accent: 'border-purple-500', badgeColor: 'bg-purple-500' },
  { id: 'Completed', label: 'Completed', accent: 'border-emerald-500', badgeColor: 'bg-emerald-500' }
];

export const TaskKanban = ({ externalSearchQuery = '' }) => {
  ...
  const fetchData = async () => {
    try {
      const [tasksRes, projectsRes, teamRes] = await Promise.all([
        api.tasks.getAll({ search: search || undefined }),
        api.projects.getAll(),
        api.team.getAll()
      ]);
  ...
  const handleDrop = async (e, targetStatus) => {
    ...
    try {
      ...
      addToast('Failed to sync drag and drop update', 'error');
      fetchData(); // Rollback
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-white dark:bg-dark-card border border-gray-100 dark:border-gray-800 shadow-soft">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            Drag and drop task cards to reassign status stages in real-time
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 text-xs bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-gray-700 rounded-2xl focus:border-brand-500 focus:outline-none w-48 sm:w-64"
            />
          </div>
          <button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center gap-2 bg-gradient-brand text-white px-5 py-2.5 rounded-2xl text-xs font-bold shadow-brand hover:opacity-95 transition-all shrink-0"
          >
      ...
      {loading ? (
        <div className="py-12 text-center text-xs text-gray-500 font-semibold">Loading Kanban Board...</div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between pb-3 mb-3 border-b border-gray-200 dark:border-gray-800">
                  ...
                  <span className="text-xs font-extrabold px-2 py-0.5 rounded-full bg-white dark:bg-dark-card text-gray-700 dark:text-gray-300 shadow-sm">
                    {colTasks.length}
                  </span>
                </div>
                ...
                <div className="flex-1 space-y-3 overflow-y-auto">
                  {colTasks.map((t) => (
                    ...
                    >
                      {/* Category / Project Name & Priority */}
